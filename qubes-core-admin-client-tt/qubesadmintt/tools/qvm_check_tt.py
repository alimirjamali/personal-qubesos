...
from qubesadmin.tools.qvm_check import print_msg
import qubesadmintt.tools

parser = qubesadmintt.tools.QubesArgumentParser(description=__doc__,
                                              vmname_nargs='+')
...
parser.add_argument("--internal", action="store_true", dest="internal",
                    default=False,
                    help="Determine if (any of given) VM are internal")
parser.add_argument("--servicevm", action="store_true", dest="servicevm",
                    default=False,
                    help="Determine if (any of given) VM are ServiceVM")
...
def get_filters(args):
    ...
    if args.internal:
        filters.append({'status': 'internal',
            'check': lambda x: x.features.get('internal', None)})
    if args.servicevm:
        filters.append({'status': 'servicevm',
            'check': lambda x: x.features.get('servicevm', None)})

    return filters


def main(args=None, app=None):
    """Main function of qvm-check-tt"""
    args = parser.parse_args(args, app=app)
    domains = args.domains
    return_code = 0

    log = args.app.log
    log.name = "qvm-check-tt"
